...
namespace VortexVise.Core.Scenes
{
    ...
    public class GameplayScene
    {
        ...
        public void InitGameplayScene<TCamera>(IAssetService assetService, IRendererService rendererService) where TCamera : IPlayerCamera, new()
        {
        ...
        public void UpdateGameplayScene(SceneManager sceneManager, ICollisionService collisionService, IRendererService rendererService)
        {
            ...
            while (simulationTime >= DeltaTime) // perform one update for every interval passed
            {
                ...
                if (GameClient.IsConnected)
                {
                    ...
                    {
                        State = GameplayLogic.SimulateState(collisionService, LastState, CurrentTime, (float)(DeltaTime - Accumulator), true, _inputService, sceneManager);
                    ...
                    else
                    {
                        ...
                        if (receivedState.Tick != GameClient.LastTickSimluated)
                        {
                            ...
                            State = GameplayLogic.SimulateState(collisionService, receivedState, CurrentTime, (float)(DeltaTime - Accumulator), true, _inputService, sceneManager);
            ...
            if (!isSlowerThanTickRate)
            {
                ...
                State = GameplayLogic.SimulateState(collisionService, LastState, CurrentTime, (float)accumulatorSimulationTime, false, _inputService, sceneManager);
                LastTimeAccumulator = CurrentTime;
                LastState = State;
            }
